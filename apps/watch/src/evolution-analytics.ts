...
export function createEvolutionAnalyticsService(deps: {
  ...
  async function build(instanceId?: string, range: "24h" | "7d" | "30d" = "7d"): Promise<EvolutionOverviewView> {
    ...
    const statusDetail = !selected ? "没有可用 Hermes 实例" : totals.sampleStatus === "insufficient" ? `样本不足（还需 ${Math.max(0, 5 - totals.completedSessions)} 个正常结束的会话）；当前工具调用 ${totals.toolCalls} 次、已完成会话 ${totals.completedSessions} 个` : statusValue === "blocked" ? "存在阻断进化的行动事项" : "观测数据持续更新中";
