...
describe("createLogAnalyzer 日志错误指纹聚合", () => {
  ...
  it("未归类 ERROR 归为系统错误并建议重启", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-08-24T10:01:00.000Z"));
    const analyzer = createLogAnalyzer(
  ...
  it("正常日志不产生问题", async () => {
    ...
    expect((await analyzer.analyze()).issues).toEqual([]);
  });

  it("支持时间窗口、轮转日志和脱敏样例", async () => {
    vi.useFakeTimers();
