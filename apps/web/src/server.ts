...
export function createWebServer(options: WebServerOptions = {}): FastifyInstance {
  ...
  for (const route of [
    ...
    app.route({
      ...route, bodyLimit: SCHEDULED_TASK_MAX_BODY,
        ...
        try {
          ...
          try {
            for (;;) {
              ...
              if (bytes > SCHEDULED_TASK_MAX_RESPONSE) { await reader.cancel(); throw new Error("invalid_response"); }
              chunks.push(value);
            }
            const parsed = parseScheduledTaskResponse(query.action, JSON.parse(Buffer.concat(chunks).toString("utf8")));
            result = parsed && (upstream.ok || parsed.reason) ? parsed : scheduledTaskFailure(query, "invalid_response");
            if ("requestId" in query && (!("requestId" in result) || query.requestId !== result.requestId)) {
  ...
  app.get("/api/llm/cost/summary", async (request, reply) => {
    ...
    const params = new URLSearchParams();
    if (typeof query["days"] === "string") params.set("days", query["days"]);
    return proxyWatchGet("/api/llm/cost/summary" + (params.size > 0 ? "?" + params.toString() : ""), reply);
  });
  ...
  const forwardAuditQuery = (request: { query?: unknown }, watchPath: string): string => {
    ...
    for (const key of ["hours", "kind", "severity", "limit"]) {
      if (typeof query[key] === "string" && query[key] !== "") params.set(key, query[key] as string);
    }
  ...
  app.get("/api/trust/report", async (_request, reply) => proxyWatchGet("/api/trust/report", reply, 60_000));
  app.get("/api/trust/report/history", async (request, reply) => {
    ...
    return proxyWatchGet("/api/trust/report/history" + (params.size > 0 ? "?" + params.toString() : ""), reply);
  });
  ...
  app.get("/api/sessions", async (request, reply) => {
    ...
    return proxyWatchGet("/api/sessions" + (params.size > 0 ? "?" + params.toString() : ""), reply, 30_000);
  });
  ...
  app.get("/api/approvals", async (request, reply) => {
    ...
    return proxyWatchGet("/api/approvals" + (params.size > 0 ? "?" + params.toString() : ""), reply, 30_000);
  });
  ...
  app.get("/api/canary/policy", async (_request, reply) => proxyWatchGet("/api/canary/policy", reply));
  app.post("/api/canary/policy", async (request, reply) =>
    proxyWatchPost("/api/canary/policy", request.body, reply, 30_000),
  );
  app.post("/api/canary/plan", async (request, reply) => proxyWatchPost("/api/canary/plan", request.body, reply, 60_000));
  app.post("/api/canary/start", async (request, reply) => proxyWatchPost("/api/canary/start", request.body, reply, 120_000));
  app.post("/api/canary/tick", async (_request, reply) => proxyWatchPost("/api/canary/tick", {}, reply, 60_000));
