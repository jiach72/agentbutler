...
Agent Butler 帮你确认 Hermes 是否可用、哪些问题需要处理，以及下一条定时任务何时执行。消息、模型、记忆、备份和升级集中管理，原始日志与诊断工具保留在高级入口。

## 从首页开始

安装后打开 `http://127.0.0.1:7531/dashboard`，先确认 Hermes 连接，再进入「定时任务」创建第一个任务，最后查看真实运行结果。连接未就绪时先完成首页引导，不会把离线显示为没有任务。

日常入口只有五个：首页、定时任务、消息通知、智能体与记忆、设置。手机底部为首页、任务、消息、设置，紧急暂停仍在顶栏。

- **定时任务**：Hermes 是唯一调度与执行引擎。时间以实际调度时区为准，不提供 Hermes CLI 未支持的单任务时区。设置中只能保存此浏览器新建任务的通知偏好，不改已有任务。
- **消息通知**：默认只看发送失败、结果未知、等待审批和用户确认。结果未知时先核实是否收到，不自动重投；已送达记录在发送历史，完整正文在详情中。
- **设置**：本机安全、定时任务默认值、备份与升级、模型与通知、专家工具。记录与报告、日志、排障和核心文件保留在折叠工具区。自进化入口需主动开启实验功能；实例联邦入口仅在检测到至少两个实例后显示。

旧页面及审批、会话、日志深链保留至少两个发布周期。旧设置链接 `?tab=about`、`?tab=preferences`、`?tab=diagnostics` 分别进入备份与升级、模型与通知、专家工具，不删除原能力。实验功能开关只控制入口展示，不替代权限校验。

> **框架支持现状（如实说明）**：Hermes 适配器为完整能力（探测/控制/消息接管/技能与记忆驱动）；
> OpenClaw 适配器目前为**实验性只读**——支持连接探测、启停、升级与快照回滚，但技能/记忆/配置
> 仅为只读盘点，且**不支持消息接管或 Hermes 定时任务**。页面中的 OpenClaw 表述以此为准。

> 当前版本：`1.0.0`（`main`）。默认只监听本机回环地址；如需跨设备访问，请同时设置 `BUTLER_ACCESS_TOKEN` 并配置发布地址。未配置口令时不要把 Web 端口暴露到不可信网络。

## 版本规则

自 `1.0.0` 正式版起，项目遵循语义化版本规范（SemVer）：`1.x.y`。

| 段 | 含义 |
|---|---|
| `主版本 (1)` | 稳定大版本（保持控制面契约、存储模型与通信协议的向后兼容） |
| `次版本 (x)` | 功能特性新增、硬件/引擎适配扩展（向下兼容） |
| `修订号 (y)` | 缺陷修复、安全性补丁与稳定性调优 |

**发版流程**：打 `v` 前缀 tag 触发（如 `git tag v1.0.0 && git push --tags`）→ CI 质量门（TS / Compose / Bridge）通过后进入 release job → 自动创建 GitHub Release。

**日常操作**：
```bash
node scripts/version.mjs next            # 生成下一版本号（按 SemVer 递增补丁号）
node scripts/version.mjs set <semver>    # 设定版本（全仓 11 个 package.json + manifest + 源码标记 + README 同步）
node scripts/version.mjs check           # 校验全仓一致（CI 质量门强校验）
```
...
浏览器访问 `http://127.0.0.1:7531/dashboard`。先按连接提示完成实例确认与连接验证，再进入「定时任务」创建任务并核实运行结果。
...
也可以直接运行安装器（封装了同样的 Docker 路径，无需先记住仓库内部脚本；要求 npm 上已发布
`agent-butler` 包——若该包尚未发布或拉取失败，请使用上方的 `git clone + scripts/deploy.sh` 路径）：
