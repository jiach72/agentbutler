      ...
      <Flex vertical gap={16}>
        ...
        {selectedInstance && (
          <Alert
            type="info"
            showIcon
            message={`当前实例：${selectedInstance.instanceId}`}
            description="保存前会先生成版本并校验文件哈希；外部修改会被阻止，避免覆盖 Agent 的最新内容。"
          />
        )}
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={9}>
                    ...
                    return (
                      <List.Item
                        style={{
                          ...
                          boxShadow: selected ? "inset 3px 0 0 var(--ant-color-primary)" : undefined,
                        }}
                      ...
                      >
                        <List.Item.Meta
                          title={
                            <Space wrap>
                              {file.label}
                              {file.key === "memory" && <Tag>只读</Tag>}
                              {!file.exists && <Tag color="default">不存在</Tag>}
                            </Space>
                          }
                          description={
                            <Flex vertical>
                              <Typography.Text type="secondary" style={{ fontSize: 12 }}>{file.pathDisplay}</Typography.Text>
          ...
          <Col xs={24} lg={15}>
            <Card
              ...
              extra={selectedFile && (
                <Space wrap>
                  <Button icon={<DownloadOutlined />} onClick={() => void download()} disabled={!selectedFile.exists}>下载</Button>
                  <Button icon={<SafetyCertificateOutlined />} onClick={() => void backup()} disabled={!selectedFile.exists}>立即备份</Button>
              ...
              {detailLoading ? (
                ...
                <Flex vertical gap={16}>
                  {selectedFile.sensitivity === "contains-secret-pattern" && (
                    <Alert type="warning" showIcon message="检测到疑似密钥或令牌模式" description="页面不会改写内容；下载前需要再次确认。" />
                  )}
                  {!selectedFile.editable && (
                    <Alert type="info" showIcon message={selectedFile.readOnlyReason ?? "该文件只读"} />
                  )}
                  <Input.TextArea
