...
function percent(value: number | null): string { return value === null ? "未知" : `${Math.round(value * 100)}%`; }
function number(value: number | null): string { return value === null ? "未知" : String(value); }
...
export function EvolutionScoreStrip({ overview }: { overview: EvolutionOverviewPayload }) {
  ...
  return <section className="evolution-score-strip" aria-label="自进化核心指标">
    <div className={`evolution-score-main is-${statusTone}`}>
      ...
      <strong>{score === null ? "样本不足" : `${score.toFixed(1)} / 100`}</strong>
    ...
    <div className="evolution-kpi"><Statistic title="最近 improvement" value={latest?.improvement === null || latest === null ? "未知" : latest.improvement} precision={3} suffix={latest?.gainScore === null || latest?.gainScore === undefined ? undefined : <StatusBadge tone={latest.gainScore >= 100 ? "ok" : latest.gainScore === 0 ? "error" : "warn"} label={`${latest.gainScore} 分`} />} /></div>
  </section>;
}
