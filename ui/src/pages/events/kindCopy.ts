...
export const EVENT_KIND_COPY: Record<string, EventKindCopy> = {
  ...
  // ── 操作审批（approvals.ts）──────────────────────────────────
  // 高危动作两类确认：gate（事前放行）阻塞执行；audit（自动发现的事后确认）
  // 不阻塞已执行的动作，超时一律自动关闭（不再说「拦截」）。
  "action-approval": {
    label: "操作审批",
    what: "高危动作的处理进展：等确认、已追认/拒绝、或超时自动关闭。",
    action: "去「记录与审批」处理；执行前的放行单确认没问题就批准，已执行的事后确认单追认或存疑。",
  },
  "action-blocked": {
    label: "超时未处理",
    what: "高危动作被拒绝了（你拒绝的，或超时自动关闭）。",
    action: "拒绝与超时是保护行为。如果其实是误拦，重新发起并批准即可。",
  },
