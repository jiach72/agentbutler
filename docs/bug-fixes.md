...
- **部署/runtime：** 已从 WSL ext4 `/home/jiach/agentbutler` 的 `codex/product-simplification-scheduled-tasks`、提交 `9a5cdfa` 完成 Compose 重建；gateway、watch、web、updater 均 healthy，宿主控制桥 active，`/api/health` 与 `/api/scheduled-tasks/status` 正常，任务列表为 13 条且 13 条 active。`bridge-healthcheck.sh` 通过；桌面与移动真实页面无水平溢出。未对真实用户任务执行任何写操作。

## 2026-09-16 - 首页把通知送达失败误报为任务失败

- **问题：** 首页新增“今日执行/失败任务”统计时，把 Hermes 的 `last_status = "delivery_failed"`（运行成功、仅通知未送达）计入失败任务，同一任务在 `/tasks` 也被标为“执行失败”。同时“今日”按宿主系统时区计算，未使用 Hermes 调度时区。
- **风险/影响：** 用户会把“任务运行正常但通知没送达”误判为任务执行失败，与 Hermes `cron list`（`delivery_failed` 显示为黄色、`failure_streak` 不变）语义矛盾；首页统计与任务列表结论也可能不一致。
- **修复范围：** 共享契约 `ScheduledTaskSummary.lastStatus` 新增 `delivery_failed`，宿主桥不再把它折叠为 `failed`，桥内状态白名单同步；首页 status 新增 `todayRunCount`（Hermes durable `executions`，按 Hermes 时区自然日）与 `failedTaskCount`（仅统计真实运行失败：`last_status` 为 `error`/`failed`，或最近一条 durable execution 为 `failed`）；首页显示“今日执行 N 次 · 失败任务 M 个”；`/tasks` 对 `delivery_failed` 显示独立 warning 标签“通知未送达”、归入“需要关注”并参与排序。
- **回归测试：** `ui/tests/scheduled-tasks.test.ts` 覆盖 `delivery_failed` 标签与排序；`ui/tests/dashboard-health.test.ts` 覆盖计数透传、状态缺失时不编造数字；`scripts/hermes-cron-host.integration.mjs` 新增隔离用例覆盖今日执行数、`delivery_failed` 不计入失败、失败任务数与列表一致且读写过程不修改 `executions.db`；`ui/tests/dashboard-wall.browser.mjs` 断言首页渲染统计行、大屏不渲染。
- **验证命令：** `corepack pnpm exec tsc -b --pretty false`；`corepack pnpm lint`；`corepack pnpm exec vitest run --maxWorkers=1 --minWorkers=1 --reporter=dot`（193 文件通过、1 跳过；1748 项通过、4 项跳过）；`corepack pnpm build`；`docker compose config --quiet`；`git diff --check`；`wsl.exe -d Ubuntu-24.04 -- bash -lc 'cd /mnt/c/Users/jiach/Documents/Agent\ Butler && HERMES_LIVE_READ=1 node --test scripts/hermes-cron-host.integration.mjs'`（8 项通过，含真实 Hermes 13 任务只读一致性）；隔离 Playwright dashboard/wall 5 个视口通过。
- **部署/runtime：** WSL ext4 `/home/jiach/agentbutler` 更新到 `2eb2749`，重装宿主控制桥后 `docker compose up -d --build --wait`，gateway、watch、web、updater 均 healthy；`bridge-healthcheck.sh` 0 个 FAIL；`/api/scheduled-tasks/status` 返回 `todayRunCount=29`、`failedTaskCount=0`、`activeCount=13`、`timezone=Asia/Shanghai`，与列表 13 条 `lastStatus` 一致且无 `prompt`/`base_url`/`last_error` 泄漏；真实 Chrome 打开 `http://127.0.0.1:7531/dashboard` 显示“今日执行 29 次 · 失败任务 0 个”，`/tasks` 显示 13 条、需要关注 0、标签均为“执行成功”，均无页面错误与水平溢出。未对真实用户任务执行任何写操作。
