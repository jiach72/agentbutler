...
export function stateBadge(state: string): ToneBadge {
  ...
  // 已停止属于规范 §1.4 的"未启动"，用 offline 而非 unknown。
  if (["stopped", "removed", "idle"].includes(s)) return { tone: "offline", label: "已停止" };
...
export function jobBadge(status: string): ToneBadge {
  switch (status) {
    case "running":
      return { tone: "unknown", label: "进行中" };
    case "done":
      return { tone: "ok", label: "已完成" };
    case "failed":
      return { tone: "error", label: "失败" };
    case "pending":
      return { tone: "unknown", label: "等待中" };
    case "cancelled":
      return { tone: "unknown", label: "已取消" };
    case "rolled-back":
      return { tone: "warn", label: "已自动回滚" };
    default:
      return { tone: "unknown", label: status || "未知" };
  }
}
...
export function channelBadge(channel: string): ToneBadge {
  ...
  return { tone: "unknown", label: channel || "未知" };
}
