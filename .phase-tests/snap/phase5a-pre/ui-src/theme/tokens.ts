 ...
 *   本文件保留全部 5 个函数，并把别名桥扩写为全量。
 *
 * 【迁移期约定】--ab-* 是 v2 新变量；--butler-* 是历史别名桥，与 --ab-* 同源同值。
 * 页面 CSS 迁到 --ab-* 之后，别名桥在阶段 5 整体删除。
 */
...
/** 间距变量刻度（--butler-space-1..7 / --ab-space-1..7）。 */
const SPACE_SCALE = space;

/** 字阶变量刻度（--butler-text-* / --ab-text-size-*）。与 v1.2 逐字一致。 */
const TEXT_SCALE = {
  xs: "12px",
  sm: "13px",
  md: "14px",
...
/**
 * 历史别名（--butler-*）：必须逐一覆盖 v1.2 桥输出的全部名称。
 * 少一个变量在 CSS 里不会报错，只会退化为继承值 —— 静默塌陷，最难排查。
 * 阶段 5 页面 CSS 迁到 --ab-* 之后，本列表整体删除。
 */
const LEGACY_COLOR_VARS: Array<[name: string, pick: (p: SemanticPalette) => string]> = [
  ["--butler-bg", (p) => p.canvas],
  ["--butler-surface", (p) => p.surface],
  ["--butler-surface-subtle", (p) => p.surface2],
  ["--butler-sunken", (p) => p.sunken],
  // v1.2 的 raised / surface-strong 都是"抬升表面"，v2 对应 surface2。
  ["--butler-raised", (p) => p.surface2],
  ["--butler-surface-strong", (p) => p.surface2],
  ["--butler-rule", (p) => p.border],
  ["--butler-rule-strong", (p) => p.borderStrong],
  ["--butler-ink", (p) => p.text],
  ["--butler-ink-soft", (p) => p.text2],
  ["--butler-ink-faint", (p) => p.text3],
  ["--butler-muted", (p) => p.text3],
  // v1.2 起 text-muted 与 muted 同源，保持同源。
  ["--butler-text-muted", (p) => p.text3],
  ["--butler-accent", (p) => p.primary],
  ["--butler-accent-hover", (p) => p.primaryHover],
  ["--butler-accent-press", (p) => p.primaryPress],
  ["--butler-accent-soft", (p) => p.primarySoft],
  ["--butler-accent-soft-border", (p) => p.primarySoftBorder],
  ["--butler-on-accent", (p) => p.onPrimary],
  ["--butler-brand", (p) => p.brand],
  ["--butler-brand-soft", (p) => p.brandSoft],
  ["--butler-brand-line", (p) => p.brandLine],
  ["--butler-ok", (p) => p.ok],
  ["--butler-ok-soft", (p) => p.okSoft],
  ["--butler-warn", (p) => p.warn],
  ["--butler-warn-soft", (p) => p.warnSoft],
  ["--butler-error", (p) => p.error],
  ["--butler-error-soft", (p) => p.errorSoft],
  ["--butler-offline", (p) => p.offline],
  ["--butler-offline-soft", (p) => p.offlineSoft],
  // v1.2 的信号青 / 暖橙强调在 v2 收敛进黄铜与提醒色。
  ["--butler-teal", (p) => p.brand],
  ["--butler-teal-soft", (p) => p.brandSoft],
  ["--butler-cinnabar", (p) => p.warn],
  ["--butler-cinnabar-soft", (p) => p.warnSoft],
  ["--butler-shadow", (p) => p.shadow1],
  ["--butler-shadow-strong", (p) => p.shadow2],
  ["--butler-focus-ring", (p) => p.focusRing],
  // 焦点描边色 = 管家蓝主色（亮 #1B4F7A / 暗 #5B9BD1）。
  // 规范 §5 要求"2px 描边 + 3px 外发光、offset 2px"，上一行是外发光，本行补描边。
  ["--butler-focus", (p) => p.primary],
];

/**
 * 仅历史别名需要、v2 语义板不覆盖的取值（通知角标、卡片高光）。
 * 刻意挡在这里而不进 SemanticPalette：它们没有 v2 语义，
 * 阶段 5 删别名桥时一并消失，不留残渣。
 */
function legacyOnlyVars(mode: ThemeMode): Record<string, string> {
  return mode === "dark"
    ? {
        "--butler-notification-badge": "#dc4a3a",
        "--butler-notification-badge-on": "#ffffff",
        "--butler-card-highlight": "inset 0 1px 0 rgb(255 255 255 / 0.05)",
      }
    : {
        "--butler-notification-badge": "#b42318",
        "--butler-notification-badge-on": "#ffffff",
        "--butler-card-highlight": "inset 0 1px 0 rgb(255 255 255 / 0.65)",
      };
}
...
export function applyThemeCssBridge(mode: ThemeMode): void {
  ...
  for (const [name, pick] of LEGACY_COLOR_VARS) vars[name] = pick(p);
  ...
  for (let i = 0; i < SPACE_SCALE.length; i += 1) {
    const value = r(SPACE_SCALE[i]);
    vars[`--ab-space-${i + 1}`] = value;
    vars[`--butler-space-${i + 1}`] = value;
  }
  for (const [key, value] of Object.entries(TEXT_SCALE)) {
    vars[`--ab-text-size-${key}`] = value;
    vars[`--butler-text-${key}`] = value;
  }

  vars["--ab-r-card"] = r(radius.card);
  vars["--ab-r-ctl"] = r(radius.control);
  vars["--ab-r-tag"] = r(radius.tag);
  vars["--butler-radius-card"] = r(radius.card);
  vars["--butler-radius-control"] = r(radius.control);

  vars["--ab-dur-fast"] = ms(motion.fast);
  vars["--ab-dur-base"] = ms(motion.base);
  vars["--ab-dur-slow"] = ms(motion.slow);
  vars["--ab-ease"] = motion.ease;
  vars["--butler-dur-fast"] = ms(motion.fast);
  vars["--butler-dur-base"] = ms(motion.base);
  vars["--butler-dur-slow"] = ms(motion.slow);
  vars["--butler-ease"] = motion.ease;

  vars["--ab-font"] = fontFamily;
  vars["--ab-mono"] = monoFontFamily;
  vars["--butler-body-font"] = fontFamily;
  vars["--butler-mono-font"] = monoFontFamily;
  vars["--butler-control-h"] = "32px";
  ...
  Object.assign(vars, legacyOnlyVars(mode));
