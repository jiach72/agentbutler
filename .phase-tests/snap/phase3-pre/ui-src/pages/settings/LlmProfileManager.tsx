 ...
 * 全部数据流与回调原样；容器层迁移到 antd Card / Row / Col / SectionHeader。
 */
    ...
    <section>
      <Flex vertical gap={16}>
        ...
        <Descriptions size="small" column={{ xs: 1, sm: 4 }}>
          ...
          <Descriptions.Item label="探针通过">{status?.activeProfiles ?? 0}</Descriptions.Item>
        ...
        <Table<Binding> size="small" rowKey="bindingId" dataSource={bindings} pagination={false} columns={[
        ...
        <Modal open={rotateId !== null} title="轮换 API Key" okText="探针并切换" cancelText="取消" confirmLoading={rotating} onCancel={() => setRotateId(null)} onOk={() => void rotate()}>
          <Paragraph type="secondary" style={{ marginBottom: 12 }}>
            新 Key 只有在真实探针成功后才会成为 active；失败时旧版本保持可用。
          </Paragraph>
