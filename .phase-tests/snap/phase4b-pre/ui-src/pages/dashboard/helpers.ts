...
export function overallBadge(overall: string | null): ToneBadge {
  switch (overall) {
    ...
    default:
      return { tone: "unknown", label: "等待检查" };
  }
}
...
export function fingerprintBadge(status: string): ToneBadge {
  ...
  if (status === "known") return { tone: "unknown", label: "已知问题" };
  return { tone: "unknown", label: status };
}
...
export function criticalProbeBadge(
  ...
  if (probe === undefined) return { tone: "offline", label: "未接入" };
  ...
  if (probe.lastStatus === "skipped") return { tone: "unknown", label: "无可检查对象" };
